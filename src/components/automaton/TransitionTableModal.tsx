...
export const TransitionTableModal: React.FC<TransitionTableModalProps> = ({
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Tabela de Transicoes" className="max-w-4xl">
            <div className="space-y-4">
                {states.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-default p-6 text-sm text-muted">
                        Adicione estados para criar transicoes.
                    </div>
                ) : (
                    <>
                        {automaton.tipo === 'AFN' && (
                            <div className="rounded-2xl border border-default bg-surface-muted px-4 py-3 text-xs text-secondary">
                                No AFN, multiplas transicoes com o mesmo simbolo representam um conjunto de destinos (I': Q x Sigma -&gt; 2^Q).
                            </div>
                        )}
                        <div className="grid grid-cols-12 gap-3 ui-kicker text-secondary">
                            <div className={isMealy ? 'col-span-3' : 'col-span-4'}>De</div>
                            <div className={isMealy ? 'col-span-3' : 'col-span-4'}>Simbolo(s)</div>
                            {isMealy && <div className="col-span-2">Saida</div>}
                                ...
                                <div key={t.id} className="grid grid-cols-12 gap-3 items-center">
                                    ...
                                    <div className="col-span-1 flex justify-end">
                                        <button
                                            ...
                                            className="p-2 rounded-lg text-ios-red hover:bg-red-50 dark:hover:bg-red-900/20"
                                            title="Remover"
                                        >
                                            <Trash2 size={16} />
                                        </button>
